...
import {
  ...
  HelpCircle, X, ExternalLink, MessageCircle, Sparkles, ListOrdered, ChevronDown, Loader2,
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { format } from 'date-fns';
import { he } from 'date-fns/locale';

// Summary-only Thank You view for ceramics ("סדנת קרמיקה") orders — order
// details + total, WhatsApp confirmation note, and a contact/support popup.
// No catalog, no sketch selection, no group/self-selection.
export default function CeramicsThankYou({
  order,
  ecomSummary,
  orderHistory,
  onSwitchOrder,
  ...
  const whatsappText = encodeURIComponent(
    `שלום! ביצעתי הרגע הזמנה לסדנת קרמיקה על שם ${ecomSummary?.buyerName || ''}, מספר ההזמנה שלי הוא ${ecomSummary?.orderNumber || ''}`
  );
  ...
  const calendarUrl = useMemo(() => {
    ...
    const title = encodeURIComponent(ecomSummary?.workshopName || 'סדנת קרמיקה - סטודיו האפי');
    ...
    <div className="py-3 space-y-3" dir="rtl">
      ...
      <div className="bg-white rounded-2xl border border-[#e8e8e8] p-3.5 shadow-sm space-y-2.5">
        <h3 className="text-lg font-bold text-[#581E83] leading-snug">
          {ecomSummary?.workshopName || 'סדנת קרמיקה'}
        </h3>
        {workshopDate && (
        ...
        <div className="flex items-center gap-1.5 text-[15px] text-[#464646]">
          ...
          <span>
            ...
            {' · '}
            <Sparkles className="w-3.5 h-3.5 inline-block text-[#5E2F88] -translate-y-0.5" /> {itemsCount} {itemsCount === 1 ? 'כלי' : 'כלים'}
          </span>
        </div>
