/* ─────────────────────────────────────────────
   Studio Happy – Rainbow Left
   Custom Element: <rainbow-left>

   הקשת נצמדת תמיד לתחתית-שמאל של המסך (fixed).
...
const STRIPE_COLORS = ["#E4C1F9", "#A9DEF9", "#D0F4DE", "#FCF6BD"];

const STRIPES = STRIPE_COLORS.map((color, index) => {
    const radius = INNER_RADIUS + STRIPE_WIDTH * (STRIPE_COLORS.length - 1 - index);
    const topY = TURN_Y - radius;
    const rightX = TURN_X + radius;

    return {
        color,
        d: `M ${START_X},${topY} L ${TURN_X},${topY} A ${radius},${radius} 0 0,1 ${rightX},${TURN_Y} L ${rightX},${BOTTOM_Y}`,
    };
});
...
class RainbowLeft extends HTMLElement {
    constructor() {
        super();
...
customElements.define("rainbow-left", RainbowLeft);
